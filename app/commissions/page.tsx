      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex rounded-lg overflow-hidden border border-slate-700">
              {DATE_OPTIONS.filter(o => o.value.startsWith('week') || o.value === 'last_week').map(o => (
                <button key={o.value} onClick={() => setDateFilter(o.value)}
                  className={`px-3 py-1.5 text-xs font-medium whitespace-nowrap transition-colors border-r border-slate-700 last:border-0 ${
                    dateFilter === o.value ? 'bg-amber-500 text-black' : 'bg-slate-900 text-slate-400 hover:text-white'
                  }`}>
                  {o.label}
                </button>
              ))}
            ...
            {/* Month group */}
            <div className="flex rounded-lg overflow-hidden border border-slate-700">
              {DATE_OPTIONS.filter(o => o.value.startsWith('month') || o.value === 'mtd' || o.value === 'last_month').map(o => (
                <button key={o.value} onClick={() => setDateFilter(o.value)}
                  className={`px-3 py-1.5 text-xs font-medium whitespace-nowrap transition-colors border-r border-slate-700 last:border-0 ${
                    dateFilter === o.value ? 'bg-amber-500 text-black' : 'bg-slate-900 text-slate-400 hover:text-white'
                  }`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
            ...
            <div className="text-2xl font-bold text-slate-400 mt-1">
              {loading ? '…' : techs.length - earners.length}
            </div>
            <div className="text-xs text-slate-500 mt-1">Not yet at $80K</div>
          </div>
          <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
            <div className="text-xs text-slate-500 uppercase tracking-wider">Payment Schedule</div>
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs text-slate-400">
            <div><span className="text-white font-medium">Rate:</span> 1.5% for doing + 1.5% for selling</div>
            <div><span className="text-white font-medium">Threshold:</span> $80,000 monthly revenue to qualify</div>
            <div><span className="text-white font-medium">Blocked if:</span> margin &lt;15%, invoice unpaid, callback</div>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <div className="w-20 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all duration-500 ${t.meetsThreshold ? 'bg-green-500' : 'bg-amber-600'}`}
